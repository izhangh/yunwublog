...
<div class="modal fade" id="<?php echo GROUP_NAME; echo MODULE_NAME; echo ACTION_NAME;?>HandleFormModal" tabindex="-1" role="dialog" aria-labelledby="<?php echo GROUP_NAME; echo MODULE_NAME; echo ACTION_NAME;?>HandleFormModalLabel">
    ...
    <form class="form-horizontal" id="<?php echo GROUP_NAME; echo MODULE_NAME; echo ACTION_NAME;?>HandleForm" name="<?php echo GROUP_NAME; echo MODULE_NAME; echo ACTION_NAME;?>HandleForm" action="<?php echo U(GROUP_NAME.'/'.MODULE_NAME.'/handle');?>" method="post">
        <div class="form-group">
            <label class="col-sm-3 control-label">规则中文名称:</label>
            <div class="col-sm-9">
                <input type="text" class="form-control" id="title" name="title" data-bv-field="title" value="" />
            </div>
        </div>
        <div class="form-group">
            <label class="col-sm-3 control-label">规则链接:</label>
            <div class="col-sm-9">
                <input type="text" class="form-control" id="name" name="name" data-bv-field="alink" value="" />
                <p class="help-block">/分组/模块/方法　/Admin/Index/index</p>
            </div>
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
            ...
            <div class="col-sm-9">
                <select name="menu_link" id="menu_link" class="form-control selectpicker" data-live-search="true">
                    <option value="" selected>不选择</option>
                    <?php if(is_array($rule_menu_list)): foreach($rule_menu_list as $key=>$v): ?><option value="<?php echo ($v["id"]); ?>"><?php echo ($v["name"]); ?></option><?php endforeach; endif; ?>
                </select>
            </div>
        </div>
    ...
    <div class="row">
        <div class="col-md-12 p15 p25">
            <div class="page-header clearfix">
                ...
                <button type="button" class="pull-right btn btn-success" data-toggle="modal" data-target="#<?php echo GROUP_NAME; echo MODULE_NAME; echo ACTION_NAME;?>HandleFormModal" data-action="add" data-backdrop="static" data-keyboard="false">添加功能</button>
            </div>
                    ...
                    <table class="table table-hover table-bordered table-advanced tablesorter tb-sticky-header">
                        <thead>
                            <tr>
                                ...
                                <th>规则名称</th>
                                <th class="hidden-xs">规则链接</th>
                                <th class="hidden-xs">所属菜单</th>
                                <th width="150">操作</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php if(is_array($list)): $key = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($key % 2 );++$key;?><tr>
                                    ...
                                    <td class="hidden-xs"><?php echo ($vo['menu_name']); ?></td>
                                    <td>
    ...
    <script>
        $('#<?php echo GROUP_NAME; echo MODULE_NAME; echo ACTION_NAME;?>HandleFormModal').on('show.bs.modal', function (event) {
            ...
            if(action == 'add') {
                ...
                $('#menu_link').selectpicker('val', dataList[obj].menu_link);
