...
<div class="container bordercomm nopadding bgfff clearfix">
    ...
    <div class="row">
        <div class="col-md-12 p15 p25">
            ...
            <div class="clearfix">
                <form class="form-horizontal" action="<?php echo U(GROUP_NAME.'/'.MODULE_NAME.'/index');?>" method="POST">
                    <div class="form-group">
                        <div class="col-xs-6 col-sm-6 col-md-4">
                            <input class="pull-left form-control" name="keys" id="keys" value="<?php echo ($_REQUEST['keys']); ?>" placeholder="请输入关键字">
                            <input class="hidden" name="channel_id" id="channel_id"  type="text" value="<?php echo ($channel_id); ?>">
                        </div>
                        ...
                        <div class="col-xs-4 col-sm-4 col-md-3 col-md-offset-3 text-right">
                            <a href="<?php echo U(GROUP_NAME.'/'.MODULE_NAME.'/page' ,array('channel_id' => $channel_id));?>"><button type="button" class="btn btn-success">添加文章</button></a>
                        </div>
                    </div>
                </form>
            </div>
                ...
                <div class="table-responsive">
                    <table class="table table-hover table-bordered table-advanced tablesorter tb-sticky-header">
                        ...
                        <?php if(is_array($list)): $key = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($key % 2 );++$key;?><tr>
                                ...
                                <td>
                                    <a href="<?php echo U('Index/Index/article', array('id' => $vo['id']));?>" target="_blank"><?php echo ($vo['name']); ?></a>
                                </td>
                                <td><?php echo date('Y-m-d', $vo['update_date']);?></td>
                                <td>
                                    <a href="<?php echo U(GROUP_NAME.'/'.MODULE_NAME.'/page' ,array('channel_id' => $channel_id ,'article_id' => $vo['id']));?>"><button type="button" class="btn btn-success">修改</button></a>
