            ...
            <div class="col-sm-10">
                <select name="pid" id="pic" data-bv-field="pic" class="form-control selectpicker" data-live-search="true">
                    ...
                    <?php if(is_array($list)): foreach($list as $key=>$v): ?><option value="<?php echo ($v["id"]); ?>">
                            <?php switch($v["level"]): case "0": ?>+<?php break;?>
    ...
    <div class="row">
        <div class="col-md-12 p15 p25">
            <div class="page-header clearfix">
                ...
                <button type="button" class="pull-right btn btn-success" data-toggle="modal" data-target="#<?php echo GROUP_NAME; echo MODULE_NAME; echo ACTION_NAME;?>HandleFormModal" data-action="add" data-backdrop="static" data-keyboard="false">添加菜单</button>
            </div>
                    ...
                    <table class="table table-hover table-bordered table-advanced tablesorter tb-sticky-header">
                        ...
                        <tbody>
                            <?php if(is_array($list)): $key = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($key % 2 );++$key;?><tr>
                                    ...
                                    <td>
                                        <?php switch($vo["level"]): case "0": ?>+<?php break;?>
                                            <?php case "1": ?>+--<?php break;?>
                                            <?php case "2": ?>+------<?php break;?>
                                            <?php default: endswitch;?>
                                        <?php echo ($vo['name']); ?>
                                    </td>
                                    <td class="hidden-xs"><?php echo ($vo['alink']); ?></td>
                                    <td>
                                        ...
                                        <?php if(($vo['is_parent']) != "1"): ?><button class="btn btn-danger <?php echo GROUP_NAME; echo MODULE_NAME; echo ACTION_NAME;?>DelBtn" data-info="您确定要删除嘛?" data-objurl="<?php echo U(GROUP_NAME.'/'.MODULE_NAME.'/del', array('id' => $vo['id']));?>">删除</button><?php endif; ?>
                                    </td>
