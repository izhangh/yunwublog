 ...
 * @return array
 */
function set_field_dict_list($table_name, $field_name) {
...
function get_system_option($name) {
    $model = D('SystemOption');
    $data = $model->field("value")->where("name = '".$name."'")->find();
    if(!empty($data)) {
        return $data['value'];
    }
    return '';
...
/**
 * 无权限时，控制按钮隐藏  <php>echo display(GROUP_NAME . MODULE_NAME.'/del');</php>
 * @param $name
 * @return string
 */
