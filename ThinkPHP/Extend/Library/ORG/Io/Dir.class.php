...
{//类定义开始
	 ...
	 * @return void
	 +----------------------------------------------------------
	 */
	function copyFile($file_name, $start) {
		if(file_exists($file_name)) {
			try {
				$code = substr($file_name, $start+	13, 1);
				if($code == '.') {
					$pre = substr($file_name, 0, $start+13);
					$ext = substr($file_name, $start+13);
					$newfile = $pre.'(10)'.$ext;
				} elseif($code == '(') {
					$pre = substr($file_name, 0, $start+13+1);
					$num = substr($file_name, $start+13+1, 2);
					$ext = substr($file_name, $start+13+1+2);
					$newfile = $pre.($num+1).$ext;
				} else {
					echo "文件命名格式错误!";
					return false;
				}
            	if(copy($file_name, $newfile)) {
					return substr($newfile, 2);
            	}
            }catch(Exception $e){
        		echo $e->getMessage();
				return false;
