...
class WechatAuth {
     ...
     * 微信多客服根路径
     */
    private $kefuApiURL = "https://api.weixin.qq.com/customservice";
    ...
    /**
     * 创建二维码，可创建指定有效期的二维码和永久二维码
     * @param  integer $scene_str      字符型二维码参数
     * @param  integer $expire_seconds 二维码有效期，0-永久有效
     */
	public function qrcodeCreateStr($scene_str, $expire_seconds = 0){
        $data = array();
     ...
     * @return array          api返回结果
     */
    protected function apiKefu($name, $data = '', $method = 'POST', $param = ''){
        $params = array('access_token' => $this->accessToken);
        ...
        $data = self::httpKefu($url, $params, $data, $method);
     ...
     * @return array          响应数据
     */
    protected static function httpKefu($url, $param, $data = '', $method = 'GET'){
        $opts = array(
            CURLOPT_TIMEOUT        => 30,
            CURLOPT_RETURNTRANSFER => 1,
            CURLOPT_SSL_VERIFYPEER => false,
        ...
        /* 根据请求类型设置特定参数 */
        $opts[CURLOPT_URL] = $url . '?' . urldecode(http_build_query($param));
     ...
     * @param  array $button 符合规则的菜单数组，规则参见微信手册
     */
    public function kefuAccountCreate($kf_account,$nickname,$password){
        $data = array('kf_account' => $kf_account,
        			  'nickname' => $nickname,
        			  'password' => md5($password));
        return $this->apiKefu('kfaccount/add', $data);
    }
	/**
     * 修改客服账号
     * @param  array $button 符合规则的菜单数组，规则参见微信手册
     */
    public function kefuAccountUpdate($kf_account,$nickname,$password){
        $data = array('kf_account' => $kf_account,
        			  'nickname' => $nickname,
        			  'password' => md5($password));
        return $this->apiKefu('kfaccount/update', $data);
    }
    /**
     * 删除客服账号
     * @param  array $button 符合规则的菜单数组，规则参见微信手册
     */
    public function kefuAccountDelete($kf_account){
        $data = array('kf_account' => $kf_account);
        return $this->apiKefu('kfaccount/del', '', 'GET', $data);
    }
	/**
     * 获取客服列表
     * @param  array $button 符合规则的菜单数组，规则参见微信手册
     ...
     * @param  array $button 符合规则的菜单数组，规则参见微信手册
     */
    public function getOnlineKefuAccountList(){
     ...
     * @param unknown_type $kf_account
     * @param unknown_type $text
     */ 
    public function createSession($openid,$kf_account,$text){
    	$data = array('openid' => $openid,
        			  'kf_account' => $kf_account,
        			  'text' => $text);
    	return $this->apiKefu("kfsession/create",$data);
    }
	/**
     * 关闭一个客服会话
     * Enter description here ...
    ...
    public function closeSession($openid,$kf_account,$text){
    	$data = array('openid' => $openid,
        			  'kf_account' => $kf_account,
        			  'text' => $text);
    	return $this->apiKefu("kfsession/close",$data);
    }
	/**
     * 获取用户的客服会话
     * Enter description here ...
    ...
    public function getSession($openid){
    	$data = array('openid' => $openid);
    	return $this->apiKefu("kfsession/getsession",'', 'GET', $data);
    }
    /*
     * 获取jsapi_ticket
     */
