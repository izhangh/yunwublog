...
class Wechat {
     ...
     * @param  string $type    消息类型
     */
    public function responseKeFu($content, $type = self::MSG_TYPE_KEFU, $KfAccount,$ToUserName,$FromUserName){
        /* 基础数据 */
        $data = array(
            'ToUserName'   => ($ToUserName ? $ToUserName : $this->data['FromUserName']),
            'FromUserName' => ($FromUserName ? $FromUserName : $this->data['ToUserName']),
            'CreateTime'   => NOW_TIME,
            'MsgType'      => $type,
        	'TransInfo'	   => array('KfAccount' => $KfAccount),
        );

        /* 按类型添加额外数据 */
        $content = call_user_func(array(self, $type), $content);
        if($type == self::MSG_TYPE_TEXT || $type == self::MSG_TYPE_KEFU || $type == self::MSG_TYPE_NEWS){
            $data = array_merge($data, $content);
        } else {
            $data[ucfirst($type)] = $content;
        }
     ...
     * @param  string $text   回复的文字
     */
	public function replyTextKefu($text,$KfAccount,$ToUserName,$FromUserName){
		return $this->responseKeFu($text, self::MSG_TYPE_KEFU,$KfAccount,$ToUserName,$FromUserName);
    }
     ...
     */
    public function replyNews($news, $news1, $news2, $news3, $news4, $news5, $news6, $news7, $news8, $news9){
