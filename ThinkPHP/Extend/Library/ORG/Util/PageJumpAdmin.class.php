...
// | Author: liu21st <>
// +-------------------------------------------------------------------------------
// | Mark: The corrected page class is just for admin page on every modules of TPD
// +-------------------------------------------------------------------------------
// $Id$
...
//			return '';
		...
		open(url,"_self"); //这里设置在那里显示 显示方式 __blank __parent __top 等 这里的 mainFrame 是在名叫 mainFrame这个框架中显示
		}
		//-->
		</script>';
		
		$up = '<select name="jumpMenu" id="jumpMenu" style="padding:1px;" onchange="MM_jumpMenu(this)">';
		$linkPage = "";
		$down = '</select>';
		$selected = "";
		...
		for($i = 1; $i <= $this->rollPage; $i ++) {
			...
			if ($page != $this->nowPage) {
				...
				if ($this->totalPages != 1) {
					...
					$linkPage .= "<option value=\"$ul\" $selected >第 1 页</option>";
				}
			}
		}
		$pageStr = str_replace ( array ('%header%', '%nowPage%', '%totalRow%', '%totalPage%', '%upPage%', '%downPage%', '%first%', '%prePage%', '%script%', '%up%', '%linkPage%', '%down%', '%nextPage%', '%end%' ), array ($this->config ['header'], $this->nowPage, $this->totalRows, $this->totalPages, $theFirst, $upPage, $downPage, $theEnd, $script, $up, $linkPage, $down, $nextPage, $prePage ), $this->config ['theme'] );
		return $pageStr;
	}
